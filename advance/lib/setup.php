...
function elgg_error_handler($errno, $errmsg, $errfile, $errline, $errcontext) {
    ...
    $fatal = "$errmsg (# $errno)";
    $file = sprintf(__gettext("Error in line %d of file %s"),$errline,$errfile);
    ...
    switch ($errno) {
        ...
        case E_COMPILE_ERROR:
            ...
            if ($CFG->debug > 0) {
                echo "<p><em>".__gettext("Disable debug mode if you do not want to display errors on screen browser")."</em></p>\n";
                echo $msg;
            } else {
                echo "<h2>".__gettext("Our apologies, the system can't complete your request")."</h2>\n";
